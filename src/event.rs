...
pub struct EventQueue {
    events: BTreeMap<SimTime, VecDeque<EventType>>,
    last_tick: SimTime,
}
...
impl EventQueue {
    pub fn new() -> Self {
        ...
        let last_tick = SimTime::from_millis(0.0);
        EventQueue { events, last_tick }
    }

    // Schedules a new event at a specific simtime.
    pub fn schedule(&mut self, delay: SimTime, event: EventType) {
...
mod tests {
    ...
    fn eventqueue_earlier_later_events_work() {
        ...
        let t1 = SimTime::from_secs(230.0);
        let t2 = SimTime::from_secs(100.0);
        ...
        assert_eq!(eq.now(), SimTime::from_secs(230.0));
    }
    ...
    fn eventqueue_queued_times_work() {
        ...
        for _ in 1..100 {
            ...
            let t = SimTime::from_millis(rand_time);
            eq.schedule(t, e.clone());
            println!("Scheduled for rand_time: {}, SimTime: {}", rand_time, t);
        }
